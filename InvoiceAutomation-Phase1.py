...
def download_attachments(msg, email_uid, email_internal_date):
    for part in msg.walk():
        ...
        if file_name:
            # Extract the file extension and convert it to lower case for comparison
            file_extension = os.path.splitext(file_name)[1].lower()
            # Define the set of allowed extensions
            allowed_extensions = {'.png', '.jpeg', '.jpg', '.pdf', '.tiff'}

            # Check if the email is new or has a newer date and the file has an allowed extension
            if (email_uid not in processed_emails or email_internal_date > processed_emails[email_uid]) and file_extension in allowed_extensions:
